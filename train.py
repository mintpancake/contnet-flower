...
# TODO: Data normalization (may require offline data augmentation);
...
class Trainer():
    def __init__(self, config_path, data_path, save_path, log_path):
        ...
        self.train_meta_path = os.path.join(data_path, 'transformed/meta/train.txt')
        self.eval_meta_path = os.path.join(data_path, 'meta/val.txt')
        self.device = 'cuda' if torch.cuda.is_available() else 'cpu'
        self.train_data = FlowersDataset(meta_file=self.train_meta_path)
    ...
    def train(self):
        ...
        for batch, (img, label) in enumerate(self.train_loader):
            ...
            if self.total_train_step % self.config["save_loss_log_steps"] == 0:
                self.loss_writer.add_scalar('Training loss', loss.item(), self.total_train_step)
                pass
    ...
    def eval(self):
        ...
        if self.total_val_step % self.config["save_acc_log_steps"] == 0:
            self.acc_writer.add_scalar('Training accuracy', val_acc, self.total_val_step)
